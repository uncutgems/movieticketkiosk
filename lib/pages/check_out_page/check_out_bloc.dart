...
class CheckOutBloc extends Bloc<CheckOutEvent, CheckOutState> {
  ...
  Stream<CheckOutState> mapEventToState(
    ...
    if (event is CheckOutEventClickButton){
      final Order order = await orderRepository.createOrder(event.customerId, event.planScreenId, event.seatsF1, event.listChairValueF1, event.customerFirstName, event.customerLastName, event.paymentMethodSystemName);
